...
sub parseArguments {
	...
	while ($i>=0) {
		...
		if ($a eq "-s") {
			...
			my $b = shift @ARGV || 
			printHelp("Bad arguments, -p needs a value, see the playlists command"); 
			$OPT_pid = int $b;
		} elsif ($a eq "-t") {		$i--;
			my $b = shift @ARGV || 
			printHelp("Bad arguments, -t needs a value, see the tracks command"); 
			$OPT_tid = int $b;
		} elsif ($a eq "-track") {	$i--;
			my $b = shift @ARGV || 
			printHelp("Bad arguments, -track needs a value"); 
			$OPT_track = $b;
		} elsif ($a eq "-artist") {	$i--;
			my $b = shift @ARGV || 
			printHelp("Bad arguments, -artist needs a value"); 
			$OPT_artist = $b;

		} elsif ($a eq "-l") {		$i--;
			my $val = shift @ARGV || 
			printHelp("Bad -l value ") ; 
			$OPT_limit = int $val; 
...
sub printHelp {
	...
	.  add:       -p pID <-track TITLE -artist NAME | -t tID>
	.  tracks,  
	.  playlists: -l, -r
