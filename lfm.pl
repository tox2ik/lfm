	...
	while (scalar @ARGV > 0) {
		...
		if (grep { $_ eq $a} @all_possible_args and  $applicable) {
			...
			if ($optim eq 'n') {
				...
				sprintf ("The option %s takes a value of type '%s' in %s mode.\n", $a, $optim, $command),
				$exit_pebkac);
			}
		...
		# -aNick, -t'Weeping Song', -pRock\ "& Roll"
			...
			printHelp('Wrong parameter', sprintf ("Illegal option -- '%s'\n", $a), $exit_pebkac);
		}
	}
	return %options;
}
...
printHelp("Invalid command.",
	"$command should be one of: @{ [sort (keys %commands, keys %commands_short) ] }", $exit_bad)
...
while ((my $flag, my $value) = each (%cliargs)){
	...
	if	($valueType eq 'i') {
	printError('Wrong parameter.', "$command $flag n must be an integer (tID)", $exit_pebkac)
...
$OPT{limit}			= $cliargs{'-l'} || OUTPUT_LIMIT;
$OPT{reverse} 		= $cliargs{'-r'} || 0;
$OPT{pid}			= $cliargs{'-p'} || 0;
$OPT{playlist}		= $cliargs{'--p'}|| "";
$OPT{tid}			= $cliargs{'-t'} || 0;
$OPT{artist}		= $cliargs{'-a'} || "";
$OPT{album}			= $cliargs{'-A'} || "";
$OPT{track}			= $cliargs{'--t'}|| "";
$OPT{name}			= $cliargs{'-n'} || "";
$OPT{description}	= $cliargs{'-d'} || "";
$OPT{order_by}		= $cliargs{'-s'} || "";
...
sub call_api20($){
	...
	if ($call eq "track.love") {
		...
		if ($OPT{artist} eq "" && $OPT{album} eq "" && $OPT{track} eq ""){ printHelp(); }
...
sub getResponse($$){
	...
	if (ref $_[0] ne 'HASH') {
		printError('No parameters', (caller(0))[3].": needs more hash reference", $exit_bad)
	}
	...
	unless ($response->is_success) {
		...
		printError('Last.fm: oops', (caller(0))[3]."($servargs{method}): ".$response->status_line , $exit_bad);
	} 
	return $response
}
...
sub printWarning($) { printError('Last.fm:', $_[0], 0); }
